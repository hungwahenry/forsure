// Fill these in on launch day.
const APP_STORE_URL = '#';
const PLAY_STORE_URL = '#';

function AppleIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" className="size-6 shrink-0">
      <path d="M16.365 1.43c0 1.14-.493 2.27-1.177 3.08-.744.9-1.99 1.57-2.987 1.57-.12 0-.23-.02-.3-.03-.01-.06-.04-.22-.04-.39 0-1.15.572-2.27 1.206-2.98.804-.94 2.142-1.64 3.248-1.68.03.13.05.28.05.43zm4.565 15.71c-.03.07-.463 1.58-1.518 3.12-.945 1.34-1.94 2.71-3.43 2.71-1.517 0-1.9-.88-3.63-.88-1.698 0-2.302.91-3.67.91-1.377 0-2.332-1.26-3.428-2.8-1.287-1.82-2.323-4.63-2.323-7.28 0-4.28 2.797-6.55 5.552-6.55 1.448 0 2.675.95 3.6.95.865 0 2.222-1.01 3.902-1.01.613 0 2.886.06 4.374 2.19-.13.07-2.383 1.37-2.383 4.19 0 3.26 2.854 4.42 2.955 4.45z" />
    </svg>
  );
}

function PlayIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" className="size-6 shrink-0">
      <path d="M4.5 3.2 16.8 12 4.5 20.8c-.3-.2-.5-.6-.5-1.1V4.3c0-.5.2-.9.5-1.1z" />
    </svg>
  );
}

function StoreBadge({
  href,
  icon,
  line1,
  line2,
}: {
  href: string;
  icon: React.ReactNode;
  line1: string;
  line2: string;
}) {
  return (
    <a
      href={href}
      target="_blank"
      rel="noreferrer"
      className="bg-foreground text-background inline-flex items-center gap-2.5 rounded-xl px-4 py-2.5 transition-opacity hover:opacity-90"
    >
      {icon}
      <span className="flex flex-col text-left leading-none">
        <span className="text-[10px]">{line1}</span>
        <span className="mt-0.5 text-base font-semibold">{line2}</span>
      </span>
    </a>
  );
}

export function AppStoreButton() {
  return (
    <StoreBadge
      href={APP_STORE_URL}
      icon={<AppleIcon />}
      line1="Download on the"
      line2="App Store"
    />
  );
}

export function PlayStoreButton() {
  return (
    <StoreBadge
      href={PLAY_STORE_URL}
      icon={<PlayIcon />}
      line1="Get it on"
      line2="Google Play"
    />
  );
}

/** Both store badges — used once the app is live. */
export function StoreButtons() {
  return (
    <div className="flex flex-wrap items-center justify-center gap-3">
      <AppStoreButton />
      <PlayStoreButton />
    </div>
  );
}
